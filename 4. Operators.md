...
-- IN: Allow the user to specify multiple values in the WHERE clause.
SELECT *
FROM employee
...
-- NOT: An expression that return NOT TRUE for some conditions.
SELECT *
FROM employee
WHERE city NOT IN ('Delhi', 'Kolkata', 'Kochi');

-- LIKE: Returns TRUE if text character matches the wildcard pattern.
SELECT *
FROM employee
WHERE first_name LIKE 'K%';

-- IF: Applies conditional logic to queries and returns a value.
SELECT
...
-- EXISTS: Test a subquery and returns TRUE if at least one record satisfies.
SELECT *
FROM employee
